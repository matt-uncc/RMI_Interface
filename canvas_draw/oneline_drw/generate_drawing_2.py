            ...
            if results_face.multi_face_landmarks:
                ...
                for face_landmarks in results_face.multi_face_landmarks:
                    for group_name, indices in FEATURE_GROUPS.items():
                        ...
                        face_feature_paths.append(norm_path)
                        for pt in feature_points:
                            cv2.circle(segmented_image, (int(pt[0]), int(pt[1])), 2, (0, 0, 255), -1)

            # --- Shirt/Neck/Other Foreground Path ---
            outline_path = extract_longest_path_from_image(segmented_image)

            # Combine: outline path first, then face features
            final_paths = [outline_path] + face_feature_paths

            csv_path = r"C:\Users\Matt\Documents\robot_RMIU\RMI_Interface\canvas_draw\oneline_drw\csv\one_line_drawing_cleaned.csv"
            if os.path.exists(csv_path):
                os.remove(csv_path)
            # Save all paths, each as a block in the CSV
            with open(csv_path, "w") as f:
                for path in final_paths:
                    np.savetxt(f, path, delimiter=",", fmt="%.2f")
                    f.write("\n")  # Separate paths by a blank line
            print(f"[INFO] Saved {len(final_paths)} paths to {csv_path}")
            break

    cap.release()
    cv2.destroyAllWindows()
    plot_all_paths(final_paths, title="Outline + Face Features")
    return final_paths
